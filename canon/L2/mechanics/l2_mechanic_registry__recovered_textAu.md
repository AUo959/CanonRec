...
| **MECH-ECO-001** War Economy & Market Flux | **IMPLEMENTED** | `tools/mech_gov_001.py` — `WarEconomyModel`, wired via `_writeback_economy`. **Pillar A.** War **scarcity** suppresses a faction's economic output while it fights; peace drives a **reconstruction boom** back toward the (territory-capped) ceiling; and a depressed economy deepens `demographic_stress` → unrest while a booming one eases it — closing the loop **war → economic depression → grievance → war** (`Economic_Stability` is the canon Public_Opinion term). Measured: the economy busts in war and booms in peace (at-war health ~0.18–0.37 of potential vs at-peace ~0.70–0.81, gap +0.44–0.54); the feedback turns without runaway (civil-war load unchanged). Observatory gates `war_economy_active`. |
| **MECH-CUL-002** Assimilation vs Local Tradition | **IMPLEMENTED** | `tools/mech_gov_001.py` — `AssimilationModel`, wired via `_writeback_assimilation`. **Pillar A (completes it).** Holding restive, recently-reconquered ground (TER-001's contested territory) is a culture-driven choice (canon §12): an **assimilationist** culture imposes identity → control now but **identity grievance** (separatist unrest later); a **tolerant** culture preserves local tradition → civic peace (a little legitimacy back). Measured: the cost is real and culture-split (both policies exercised every run; ~115–238 assimilations vs ~52–190 tolerations across seeds), civil-war load stays under the collapse reference. *Honest note:* gentle by design — the policy split is unambiguous, the galaxy-level outcome modest, not coefficient-forced. Observatory gates `cultural_cost_active`. |
...
reproducible decision trace. Tests: `tests/test_mech_gov_001.py` (22) +
`tests/test_gumas_consequence_layer.py` (4) +
`tests/test_observatory_240_cycle.py` (16).
