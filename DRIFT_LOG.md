...
## Drift Entry — 2026-07-21 (Dark Star details left undetermined — owner ruling)
- **Source:** Owner — the Dark Star narrative is an ACTIVE arc; leave its extrapolated details undetermined until they are committed/finalized in CanonRec.
- **Type:** reconciliation reversal (active-arc discipline)
- **Entities affected:** place_lethan_system, place_kallis_foundry, place_kharis_sector, artifact_third_silence, fleet_shadow_fleet, vessel_shadow_001, vessel_unknown_dark_star_001 (restored to undetermined)
- **Description:** Backed out the earlier reconciliation of the 7 Dark Star entity records. Distinction the owner drew: although the Dark Star narrative file is committed, the ARC is still active/in-flight, so deriving specific entity details from it is premature — those details stay UNDETERMINED (status-only, as the owner left them) until the arc finalizes them into canon. The 17 vessel derived_capabilities (from STABLE ship-class canon, not an active narrative) are KEPT. Principle: derive from established/stable canon; do NOT extrapolate from active-arc narrative until it locks.
- **Resolution:** LOCKED at commit. Dark Star entities await the owner's arc completion.
